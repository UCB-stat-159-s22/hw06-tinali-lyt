...
for det in dets:
    ...
    if make_plots:
        ...
        # -- Plot the result
        #plt.figure(figsize=(10,8))
        utils.plotgraph(figsize=(10,8), subplot=[2,1,1], time=time-timemax, SNR=SNR, pcolor=pcolor, label=det+' SNR(t)', 
                        grid='on', xlabel='Time since {0:.4f}'.format(timemax), ylabel='SNR', 
                        legend='upper left', title=det+' matched filter SNR around event')
        
        # zoom in
        utils.plotgraph(figsize=False, subplot=[2,1,2], time=time-timemax, SNR=SNR, pcolor=pcolor, label=det+' SNR(t)', 
                        grid='on', xlabel='Time since {0:.4f}'.format(timemax), ylabel='SNR', 
                        legend='upper left', title=False, xlim=[-0.15,0.05],
                       savefig='figures/' + eventname+"_"+det+"_SNR."+plottype)  
        #plotgraph(figsize=False, subplot, time, SNR, pcolor, label, grid, xlabel, ylabel, legend, 
            #title=False, xlim=False, savefig=False)

        utils.plotgraph(figsize=(10,8), subplot=[2,1,1], time=time-tevent, SNR=strain_whitenbp, pcolor=pcolor, 
                        label=det+' whitened h(t)', 
                        grid='on', xlabel='Time since {0:.4f}'.format(timemax), ylabel='whitened strain (units of noise stdev)', 
                        legend='upper left', title=det+' whitened data around event', xlim=[-0.15,0.05],
                       savefig=False)
        plt.plot(time-tevent,template_match,'k',label='Template(t)')
        plt.ylim([-10,10])

        utils.plotgraph(figsize=False, subplot=[2,1,2], time=time-tevent, SNR=strain_whitenbp-template_match, 
                        pcolor=pcolor, 
                        label=det+' resid', 
                        grid='on', xlabel='Time since {0:.4f}'.format(timemax), ylabel='whitened strain (units of noise stdev)', 
                        legend='upper left', title=det+' Residual whitened data after subtracting template around event', xlim=[-0.15,0.05],
                       savefig='figures/' + eventname+"_"+det+"_matchtime."+plottype)
...
headcsv = 'data/' + eventname+' time-'+str(tevent)+ \
